...
def setup_logger(log_file: Optional[Path] = None):
    ...
    _logger = logging.getLogger("SoVITS-Server")
    _logger.setLevel(logging.INFO)
    
    if not _logger.handlers:
        console_handler = logging.StreamHandler(sys.stdout)
        console_handler.setFormatter(logging.Formatter('[%(asctime)s] [SoVITS-Server] %(message)s', '%H:%M:%S'))
...
def log(message: str):
    ...
    if _logger is None:
        ...
        print(f"[{timestamp}] [SoVITS-Server] {message}")
    else:
...
class SoVITSServer:
    ...
    def _start_broadcast(self):
        ...
        try:
            ...
            log(f"Broadcast started on port {BROADCAST_PORT}, local IP: {self._local_ip}")
        except Exception as e:
            log(f"Failed to start broadcast: {e}")
    ...
    def scan_packs(self) -> Dict[str, PackWeights]:
        ...
        if not packs_dir.exists():
            log(f"Packs directory not found: {packs_dir}")
            return {}
        ...
        for pack_dir in packs_dir.iterdir():
            ...
            try:
                ...
                log(f"Failed to read pack.json in {pack_dir.name}: {e}")
            ...
            log(f"Indexed pack: {pack_id} (version={version}, {status})")
    ...
    async def start_sovits(self, pack_id: str) -> bool:
        if self.sovits_manager:
            ...
            log(f"start_sovits check: sovits_manager exists, is_running={is_running}")
            if is_running:
                return True
        else:
            log(f"start_sovits check: sovits_manager is None")

        if self._sovits_starting:
            log(f"start_sovits check: SoVITS is already starting, waiting...")
            import time
            for _ in range(60):  
                if self.sovits_manager and self.sovits_manager.is_running():
                    log(f"start_sovits check: SoVITS is now ready")
                    return True
                time.sleep(0.5)
            log(f"start_sovits check: Timeout waiting for SoVITS to start")
            return False

        if pack_id not in self.packs_index:
            log(f"Pack not found: {pack_id}")
            return False

        weights = self.packs_index[pack_id]
        if not weights.valid:
            log(f"Pack has no valid weights: {pack_id}")
            return False
        ...
        try:
            ...
            if success:
                ...
                log(f"SoVITS process started with pack: {pack_id}")
                log(f"Waiting for SoVITS API to be fully ready...")
                import time
                for attempt in range(30):  
                    if self.sovits_manager.is_running(timeout=1.0):
                        log(f"SoVITS API is ready after {attempt + 1} checks")
                        return True
                    time.sleep(0.5)
                log(f"Warning: SoVITS API may not be fully ready yet")
            else:
                log(f"Failed to start SoVITS for pack: {pack_id}")
    ...
    async def synthesize(self, pack_id: str, text: str, emotion_config: dict, params: dict) -> Optional[bytes]:
        ...
        if not ref_wav_path:
            log("Missing ref_wav_path in emotion_config")
            return None
        ...
        if weights and weights.gpt_path and weights.sovits_path:
            ...
            log(f"Dynamic model switch: gpt={payload['gpt_path']}, sovits={payload['sovits_path']}")
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    log(f"Invalid JSON from {client_id}: {e}")
    ...
    async def handle_message(self, session: ClientSession, data: dict):
        ...
        log(f"Received message from {session.client_id}: type={msg_type}")
        ...
        if msg_type == "handshake":
            ...
            log(f"Handshake from {session.client_id}: pack={pack_id}, valid={pack_valid}")
    ...
    async def _queue_synthesize(self, session: ClientSession, data: dict):
        ...
        log(f"Queueing synthesize request: pack_id={pack_id}, session_pack={session.pack_id}")
        if not pack_id:
            await session.websocket.send(json.dumps({
        ...
        log(f"Queued synthesize request from {session.client_id} for pack {pack_id}")
    ...
    async def _process_queue(self):
        ...
        try:
            while not self._request_queue.empty():
                ...
                log(f"Processing synthesize: pack={pack_id}, request_id={request_id}, text_len={len(text)}")
                ...
                if audio_data:
                    ...
                    log(f"Sent audio to {session.client_id}: {len(audio_data)} bytes")
                else:
                    await session.websocket.send(json.dumps({
                        "type": "synthesize_result",
                        "request_id": request_id,
    ...
    async def run(self):
        ...
        log(f"Scanned {len(self.packs_index)} packs, {sum(1 for w in self.packs_index.values() if w.valid)} valid")
        ...
        if self.default_pack:
            if self.default_pack in self.packs_index and self.packs_index[self.default_pack].valid:
                log(f"Preloading default pack: {self.default_pack}")
                success = await self.start_sovits(self.default_pack)
                if success:
                    log(f"Default pack {self.default_pack} loaded successfully")
                else:
                    log(f"Failed to preload default pack: {self.default_pack}")
            else:
                log(f"Default pack {self.default_pack} not found or invalid, skipping preload")
        else:
            valid_packs = [p for p, w in self.packs_index.items() if w.valid]
            if valid_packs:
                auto_pack = valid_packs[0]
                log(f"Auto-preloading first valid pack: {auto_pack}")
                success = await self.start_sovits(auto_pack)
                if success:
                    log(f"Auto-loaded pack: {auto_pack}")
                else:
                    log(f"Failed to auto-load pack: {auto_pack}")
        ...
        try:
            async with websockets.serve(self.handle_client, "0.0.0.0", self.port):
                log(f"Server started. Local IP: {self._local_ip}:{self.port}")
